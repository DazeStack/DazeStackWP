...
### Added
- Redis Unix socket support: Redis now listens only on a Unix domain socket (`unixsocket` with `unixsocketperm 770`), TCP port disabled (`port 0`), for lower cache latency.
- WordPress Redis “steroid” options: `WP_REDIS_ASYNC_FLUSH`, `WP_REDIS_DISABLE_METRICS`, `WP_REDIS_DISABLE_BANNERS`, `WP_REDIS_TIMEOUT`, `WP_REDIS_READ_TIMEOUT` for better background flush and timeouts.
- `upgrade-sites` now migrates existing sites to Redis Unix socket config: removes `WP_REDIS_HOST`/`WP_REDIS_PORT`, sets `WP_REDIS_SCHEME`/`WP_REDIS_PATH` and the steroid flags.
...
### Changed
- Redis: removed `bind 127.0.0.1` and TCP port; credentials and `show-credentials` now show Redis Socket instead of Host/Port; `www-data` added to `redis` group and PHP-FPM restarted after Redis phase so PHP can use the socket.
- WordPress Redis config now uses `WP_REDIS_SCHEME` (unix) and `WP_REDIS_PATH` instead of `WP_REDIS_HOST`/`WP_REDIS_PORT`; all `redis-cli` invocations use `-s` (socket).

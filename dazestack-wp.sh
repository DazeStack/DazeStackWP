...
quote_sql_ident() {
    ...
    local escaped=$(printf "%s" "$ident" | sed 's/`/``/g')
    printf "\`%s\`" "$escaped"
}
...
create_site() {
    ...
    local q_db_name=$(quote_sql_ident "$db_name")
    local q_db_user=$(quote_sql_ident "$db_user")
    local q_db_pass=$(quote_sql_str "$db_pass")

    if ! mysql_exec "CREATE DATABASE IF NOT EXISTS $q_db_name CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>/dev/null; then
        log_error "Failed to create database: $db_name"
        execute_rollback
        return 1
    fi
    push_rollback "mysql_exec \"DROP DATABASE IF EXISTS ${q_db_name//\`/\\\`};\" 2>/dev/null || true"

    # Create MySQL user with limited privileges
    log_info "Creating MySQL user..."
    mysql_exec "CREATE USER IF NOT EXISTS $q_db_user@'localhost' IDENTIFIED BY $q_db_pass;" 2>/dev/null || true
    mysql_exec "GRANT ALL PRIVILEGES ON $q_db_name.* TO $q_db_user@'localhost';" 2>/dev/null
    mysql_exec "FLUSH PRIVILEGES;" 2>/dev/null
    push_rollback "mysql_exec \"DROP USER IF EXISTS ${q_db_user//\`/\\\`}@'localhost';\" 2>/dev/null || true"
...
delete_site() {
    ...
    local q_db_name=$(quote_sql_ident "$db_name")
    local q_db_user=$(quote_sql_ident "$db_user")
    mysql_exec "DROP DATABASE IF EXISTS $q_db_name;" 2>/dev/null || log_warn "Failed to drop database"
    mysql_exec "DROP USER IF EXISTS $q_db_user@'localhost';" 2>/dev/null || log_warn "Failed to drop user"
